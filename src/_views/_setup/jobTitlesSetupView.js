...
import {deleteJobTitle, createJobTitle, updateJobTitle} from '../../_actions/accountConfigurationItemsActions'
...
function JobTitlesSetupView(props)
{
    ...
    const deleteJobTitle = jobTitle => {  
      ...
      if(jobTitle)
      {
        ...
        setDialogProps({...dialogProps, dialogTitle: "Delete Job title", 
        dialogMessage: "Delete job title: "+jobTitle.name,
        okAction:()=>{props.deleteJobTitle(jobTitle);setOpenAlert(false)}
        })
      }
    ...
    return(
        ...
        Job Titles
            ...
            <PlainButton onClick={handleClickOpen}>
              <AddIcon className={classes.customButtonIcon}/>Create New Job Title
            </PlainButton>
            ...
            <Dialog open={open} fullWidth  className={classes.modalDialog} onClose={handleClose} aria-labelledby="form-dialog-title">
                ...
                {isEditing ? (<span>Edit Job Title</span>
                ...
                {isEditing ? (
                  <Button onClick={updateJobTitle} color="primary">
                    Update
                  </Button>
                ): (
                  <Button onClick={addJobTitle} color="primary">
                    Create
                  </Button>
                )}  
        ...
        <Grid container spacing={1}  className={classes.listContainer}>
            {(jobTitles && jobTitles.length >0) ?                
              jobTitles.filter( f=> searchText == ""? true:f.name.indexOf(searchText) > -1).map(jobTitle => 
                <Grid item key={jobTitle.id} xs={12}>
                <Paper key={jobTitle.id} className={classes.paper}>
                <Grid container  direction="row"
  justify="space-between" spacing={2}>
                
                <Grid item xs>
                    <Typography variant="h6" noWrap>{jobTitle.name}</Typography>                 
                </Grid>
                <Grid item xs container justify="flex-end" alignItems="center">                  
                  <IconButton size="small" onClick={() => editJobTitle(jobTitle)} color="primary">
                    <EditIcon  className={classes.iconButton2} />
                  </IconButton>
                  <IconButton size="small" onClick={() => deleteJobTitle(jobTitle)} color="primary">
...
const mapDispatchToProps = (dispatch) => {
  return {
    deleteJobTitle:(id) => {dispatch(deleteJobTitle(id))},
    createJobTitle:(jobTitle) => {dispatch(createJobTitle(jobTitle))},
    updateJobTitle:(jobTitle) => {dispatch(updateJobTitle(jobTitle))},
  }
}

export default withSnackbar(connect(mapStateToProps, mapDispatchToProps)(JobTitlesSetupView));