...
import {deleteDepartment, createDepartment, updateDepartment} from '../../_actions/accountConfigurationItemsActions'
...
function DepartmentSetupView(props)
{
    ...
    const deleteDepartment = department => {  
      ...
      if(department)
      {
        ...
        setDialogProps({...dialogProps, dialogTitle: "Delete department", 
        dialogMessage: "Delete department: "+department.name,
        okAction:()=>{props.deleteDepartment(department);setOpenAlert(false)}
        })
      }
    ...
    const handleSearchInputChange = event => {
      ...
      console.warn(value)
      console.warn(departments.filter( f=> searchText == ""? true:f.name.indexOf(searchText) > -1))
      setSearchText(value)  
    }
    ...
    function addDepartment() {     
        ...
        props.createDepartment({...newDepartment,id:Math.random()});
        setOpen(false);
    }
    ...
    return(
        ...
        Departments
            ...
            <PlainButton onClick={handleClickOpen}>
              <AddIcon  className={classes.customButtonIcon}/>Create New Department
            </PlainButton>
            ...
            <Dialog open={open} fullWidth  className={classes.modalDialog} onClose={handleClose} aria-labelledby="form-dialog-title">
                ...
                {isEditing ? (<span>Edit Department</span>
                ...
                {isEditing ? (
                  <Button onClick={updateDepartment} color="primary">
                    Update
                  </Button>
                ): (
        ...
        <Grid container spacing={1}  className={classes.listContainer}>
            {(departments && departments.length >0) ?                
              departments.filter( f=> searchText == ""? true:f.name.indexOf(searchText) > -1).map(department => 
                <Grid item key={department.id} xs={12}>
                <Paper key={department.id} className={classes.paper}>
                <Grid container  direction="row"
  justify="space-between" spacing={2}>
                
                <Grid item xs>
                    <Typography variant="h6" noWrap>{department.name}</Typography>                 
                </Grid>
                <Grid item xs container justify="flex-end" alignItems="center">                  
                  <IconButton size="small" onClick={() => editDepartment(department)} color="primary">
                    <EditIcon  className={classes.iconButton2} />
                  </IconButton>
                  <IconButton size="small" onClick={() => deleteDepartment(department)} color="primary">
...
export default withSnackbar(connect(mapStateToProps, mapDispatchToProps)(DepartmentSetupView));