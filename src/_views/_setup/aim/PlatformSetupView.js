...
import {createPlatform, updatePlatform, deletePlatform} from '../../../_actions/anomalyActions'
...
function PlatformSetupView(props)
{
    ...
    const deletePlatform = platform => {  
      ...
      if(platform)
      {
        ...
        setDialogProps({...dialogProps, dialogTitle: "Delete Facility", 
        dialogMessage: "Delete Facility: "+platform.name,
        okAction:()=>{props.deletePlatform(platform);setOpenAlert(false)}
        })
      }
    ...
    const handleSearchInputChange = event => {
      ...
      console.warn(value)
      console.warn(platforms.filter( f=> searchText == ""? true:f.name.indexOf(searchText) > -1))
      setSearchText(value)  
    }
    const handleInputChange = event => {
      const { name, value } = event.target  
      setNewPlatform({ ...newPlatform, [name]: value })      
    }

    function addPlatform() {     
        
        props.createPlatform({...newPlatform,id:Math.random()});
        setOpen(false);
    }
    ...
    return(
        ...
        Facilities
            ...
            <PlainButton onClick={handleClickOpen}>
              <AddIcon className={classes.customButtonIcon}/>Create New Facility
            </PlainButton>
            ...
            <Dialog open={open} fullWidth  className={classes.modalDialog} onClose={handleClose} aria-labelledby="form-dialog-title">
                ...
                {isEditing ? (<span>Edit Facility</span>
                ...
                {isEditing ? (
                  <Button onClick={updatePlatform} color="primary">
                    Update
                  </Button>
                ): (
                  <Button onClick={addPlatform} color="primary">
                    Create
                  </Button>
                )}  
        ...
        <Grid container spacing={1} className={classes.listContainer}>
            {(platforms && platforms.length >0) ?                
              platforms.filter( f=> searchText == ""? true:f.name.indexOf(searchText) > -1).map(platform => 
                <Grid item key={platform.id} xs={12}>
                <Paper key={platform.id} className={classes.paper}>
                <Grid container  direction="row"
  justify="space-between" spacing={2}>
                
                <Grid item xs>
                    <Typography variant="h6" noWrap>{platform.name}</Typography>                 
                </Grid>
                <Grid item xs container justify="flex-end" alignItems="center">                  
                  <IconButton size="small" onClick={() => editPlatform(platform)} color="primary">
                    <EditIcon  className={classes.iconButton2} />
                  </IconButton>
                  <IconButton size="small" onClick={() => deletePlatform(platform)} color="primary">
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    createPlatform:(user) => {dispatch(createPlatform(user))},
    updatePlatform:(user) => {dispatch(updatePlatform(user))},
  }
}

export default withSnackbar(connect(mapStateToProps, mapDispatchToProps)(PlatformSetupView));