...
const AnomaliesTable = (props) => {
  ...
  const downloadExcelFile = () => {
    downloadFile(
      "/api/anomalies/download/all_anomalies",
      "Anomalies.xlsx",
      "Excel"
    );
  };
    ...
    if (props.showOnlyOverDue) {
      setRowData(
        props.anomalies.filter(
          (i) =>
            i.status != "CLOSED" &&
            moment(new Date(i.closeOutDate)) <= moment(new Date())
        )
      );
      setFilteredRowData(
        props.anomalies.filter(
      ...
      setRowData(props.anomalies.filter((i) => i.status == "CLOSED"));
      setFilteredRowData(
        props.anomalies.filter((i) => i.status == "CLOSED")
      );
    } else {
...
export default connect(mapStateToProps)(AnomaliesTable);
