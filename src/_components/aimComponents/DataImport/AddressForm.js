...
const steps = ['Upload', 'Data selection', 'Map Columns', 'Preview Anomalies'];
            ...
            <React.Fragment>
                ...
                alignItems="center">
                    {/*<Typography>{selectedFiles.length} Selected file(s)</Typography> */}
                        ...
                        {isExcelUploaded && activeSpreadsheet && <MyComponent data={activeSpreadsheet}/>}
                        </div>
          ...
          case 1:
            return (
                <React.Fragment>
                    ...
                    <Grid item xs={12} sm={6}>Header Row Number</Grid>
                    <Grid item xs={12} sm={6}><Select
                                        ...
                                        >
                                        {activeSpreadsheet && activeSpreadsheet.map( (columnHeader, index )=> {
                    ...
                    {<MapperComponent mapObjects={modelMapFields} columnHeaders={activeSpreadsheetHeader}/>}     
                    </Grid>
              </React.Fragment>);
          case 2:
            return (
    ...
    return (
        <TableContainer component={Paper}>
            <Table>
                ...
                {modelMapFields.map( m => {
                   return <TableCell style={{color:"#FFFFFF", fontWeight:'bold'}}>{m.fieldName}</TableCell>
                })}                              
                </TableRow>
                </TableHead>
                <TableBody>
            {extractedAnomalies && extractedAnomalies.map(em =>
                 <TableRow style={{backgroundColor:"#FFFFFF", color:'#FFFFFF', borderRadius:5}}>
                       {modelMapFields.map( m => { 
                           return  ( em[m.name] && <TableCell style={{color:"#0099FF", fontWeight:'bold'}}>
      ...
      return (
        <TableContainer component={Paper}>
            <Table>
                <TableHead >
                <TableRow style={{backgroundColor:"#333333", color:'#FFFFFF', borderRadius:5}}>
                    <TableCell style={{color:"#FFFFFF", fontWeight:'bold'}}>Field</TableCell>
                    <TableCell style={{color:"#FFFFFF", fontWeight:'bold'}}>Column</TableCell>
                        
                </TableRow>
                </TableHead>
                <TableBody>
            {mapObjects && mapObjects.map(mapObject =>
                 <TableRow style={{backgroundColor:"#FFFFFF", color:'#FFFFFF', borderRadius:5}}>
                     <TableCell style={{color:"#0099FF", fontWeight:'bold'}}>
                    ...
                    <Select
                            ...
                            {columnHeaders && columnHeaders.map( (columnHeader, index )=> {
                                return <MenuItem value={index}>{columnHeader.value}</MenuItem>
                              })
                            }
                            </Select>
                            </TableCell>
     ...
     <Stepper activeStep={activeStep} className={classes.stepper}>
      ...
      <Grid container spacing={3}>
        ...
        {spreadsheets && spreadsheets.map( spreadsheet => {
            return <Button onClick={()=>setActiveSpreadsheet(spreadsheet.data)}>{spreadsheet.name}</Button>
        })}
        </Grid> 
...
const mapDispatchToProps = (dispatch) => {
    return {
        addAnomalies:(data) => {dispatch(addAnomalies(data))},    
    }
  }
...
const mapStateToProps = state => {
  ...
  export default connect(mapStateToProps, mapDispatchToProps)(withSnackbar(AddressForm));