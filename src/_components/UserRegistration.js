...
function UserRegistration(props) {
  ...
  const [identifierVisibilityState, setIdentifierVisibilityState] = React.useState(
    true
  );

  const [identifierVisibility, setIdentifierVisibility] = React.useState(
    "none"
  );
    ...
    <Container component="main" maxWidth="xs">
      ...
      <div className={classes.paper}>
        ...
        <Formik
                    ...
                    onSubmit={({companyidentifier, firstname, lastname, username, password , isAdmin}, { setStatus, setSubmitting }) => {
                        setStatus();
                        ...
                        const requestOptions = {
                            ...
                            data: {companyidentifier, firstname, lastname, username, password , isAdmin}
                        };
                        ...
                        axios(requestOptions)
                            .then((response) => {
                                ...
                                //console.log(this)
                                //setDialogOpen(true);
                                props.dispatch({type:'ENQUEUE_SNACKBAR', notification:{message: "Account created...",
                                    options: {
                           ...
                           }).catch((e)=>
                            {
                                ...
                                setStatus(e);
                                console.log(e)
                                props.dispatch({type:'ENQUEUE_SNACKBAR', notification:{message: "Unable to create account",
                                    options: {
                                        key: new Date().getTime() + Math.random(),
                                        variant: 'error'}}})
                            });
                    ...
                    render={(props) => 
                        {
    const {
        values: { companyidentifier, firstname, lastname, username, password },
        errors,
        touched,
        handleSubmit,
        handleChange,
        ...
        return (<form  onSubmit={handleSubmit} style={classes}>
          
          <Grid container spacing={2}>

          <Grid item xs={12}>
            <FormControlLabel name="isAdmin"  onChange={handleChange} 
            control={<Checkbox style={{color: '#1c75bc'}}/>} label="I'm an Administrator" />
            ...
            <Grid item xs={12}>
              <FormControlLabel
                control={<Checkbox value="allowExtraEmails" style={{color: '#1c75bc'}} />}
          ...
          <Grid container justify="flex-end">
            <Grid item>
              <Link href="#/login" style={{fontSize:16}} variant="body2">
